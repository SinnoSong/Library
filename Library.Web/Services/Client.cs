...
public class Client : IClient
{
        ...
        string? json = null, Dictionary<string, object>? queryPairs = null)
    {
        if (queryPairs != null)
        {
            ...
            foreach (var (key, value) in queryPairs) queryParams.Append(key + "=" + value + "&");
    ...
    public async Task<bool> UpdateUserGrade(Guid userId, byte grade)
    {
        var apiUrl = $"{ApplicationUrl}{Apis.UpdateUserGrade}{userId}";
        var accessToken = await _localStorageService.GetAccessTokenAsync();
        var dict = new Dictionary<string, object>
        {
            {"grade", grade}
        };
        return await SendRequest(HttpMethod.Put, apiUrl, accessToken, queryPairs: dict);
    }

    public async Task<bool> ChangeEmail(Guid userId, string newEmail)
    {
        var apiUrl = $"{ApplicationUrl}{Apis.ChangeEmail}{userId}";
        var accessToken = await _localStorageService.GetAccessTokenAsync();
        var dict = new Dictionary<string, object>
        {
            {"newEmail", newEmail}
    ...
    public async Task<bool> ChangePassword(Guid id, UserPasswordChangeDto userPasswordChangeDto)
    {
        var apiUrl = $"{ApplicationUrl}{Apis.ChangePassword}{id}";
        var accessToken = await _localStorageService.GetAccessTokenAsync();
        return await SendRequest(HttpMethod.Put, apiUrl, accessToken,
    ...
    public async Task<bool> AddAdministrator(UserDto userDto)
    {
        ...
        var accessToken = await _localStorageService.GetAccessTokenAsync();
        return await SendRequest(HttpMethod.Post, apiUrl, accessToken, JsonConvert.SerializeObject(userDto));
    }

    public async Task<List<UserDto>> GetUsers(UserQueryParameters parameters)
    {
        var apiUrl = ApplicationUrl + Apis.GetUsers;
        var accessToken = await _localStorageService.GetAccessTokenAsync();
        var dict = parameters.ToDictionary();
        return await SendRequest<List<UserDto>>(HttpMethod.Get, apiUrl, accessToken, queryPairs: dict);
    }
